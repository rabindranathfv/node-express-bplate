...
describe('User routes', () => {
  ...
  describe('GET /v1/users', () => {
    ...
    test('should return 200 if a admin is trying to access all users', async () => {
      await insertUsers([userOne, userTwo, admin]);

      await request(app).get('/v1/users').send().expect(httpStatus.OK);
    });

    test('should correctly apply filter on name field', async () => {
      await insertUsers([userOne, userTwo, admin]);

      const res = await request(app).get('/v1/users').query({ name: userOne.name }).send().expect(httpStatus.OK);
    ...
    test('should correctly apply filter on role field', async () => {
      ...
      const res = await request(app).get('/v1/users').query({ role: 'user' }).send().expect(httpStatus.OK);
    ...
    test('should correctly sort the returned array if descending sort param is specified', async () => {
      ...
      const res = await request(app).get('/v1/users').query({ sortBy: 'role:desc' }).send().expect(httpStatus.OK);
    ...
    test('should correctly sort the returned array if multiple sorting criteria are specified', async () => {
      ...
      const res = await request(app).get('/v1/users').query({ sortBy: 'role:desc,name:asc' }).send().expect(httpStatus.OK);
  ...
  describe('PATCH /v1/users/:userId', () => {
    ...
    test('should return 404 if admin is updating another user that is not found', async () => {
      ...
      await request(app).patch(`/v1/users/${userOne._id}`).send(updateBody).expect(httpStatus.NOT_FOUND);
    });
